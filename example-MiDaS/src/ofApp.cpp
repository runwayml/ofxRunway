...
void ofApp::update(){
	...
	runway.get("depth_map", runwayResult);
}
//--------------------------------------------------------------
void ofApp::draw(){
	
	// draw camera feed
	if(bDrawInput) grabber.draw(0,0);
	
	// draw image received from Runway
	if(bDrawDepth.get() && runwayResult.isAllocated()){
		runwayResult.draw(640,0);
	}
	
	
	if(bDrawStatus)runway.drawStatus(660, 520, true);
	...
	if(bDrawPoints)
	{
		...
		shader.setUniform1f("depth_scale", -1* depthScale.get());
	...
	if(bDrawGui)
	{
		...
		p.y += 20;
		ofDrawBitmapStringHighlight("press key 'g' to show/hide this GUI", p);
	}
}
//--------------------------------------------------------------
